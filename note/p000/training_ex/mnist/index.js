...
function createConvModel() {
  // 합성곱 신경망의 첫번째 층은 28x28 픽셀의 흑백이미지를 받음
  ...
  model.add(tf.layers.dense({ units: 64, activation: "relu" })); // 0 이하는 0으로 표현
  model.add(tf.layers.dense({ units: 10, activation: "softmax" })); // 결과의 합이 1이 되도록함
  return model;
}
